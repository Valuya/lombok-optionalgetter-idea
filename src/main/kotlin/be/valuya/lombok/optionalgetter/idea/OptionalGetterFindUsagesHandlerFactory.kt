...
class OptionalGetterFindUsagesHandlerFactory(private val project: Project) : FindUsagesHandlerFactory() {
    ...
    override fun canFindUsages(element: PsiElement): Boolean {
        ...
        if (element is PsiField && result) LOG.debug("OptionalGetter: canFindUsages=true for field '${element.name}'")
        return result
    }
    ...
    override fun createFindUsagesHandler(element: PsiElement, forHighlightUsages: Boolean): FindUsagesHandler? {
        ...
        LOG.debug("OptionalGetter: createFindUsagesHandler for field '${element.name}', generated=${OptionalGetterSupport.findGenerated(element)?.name}")
        return object : JavaFindUsagesHandler(element, javaFactory) {
            override fun getSecondaryElements(): Array<PsiElement> {
                ...
                LOG.debug("OptionalGetter: getSecondaryElements base=${base.map { (it as? PsiField)?.name ?: it.toString() }} generated=${generated?.name}")
